  ...
  module holes() {
    translate([0,-sheet_pos_z,0]) {
      ...
      for(side=[left,right]) {
        ...
        translate([return_line_x*side,return_line_z,0]) {
          line_hole();
        }

        translate([xy_line_x*side,to_front_line_z,0]) {
          line_hole();
        }
...
module rear_sheet() {
  ...
  module holes() {
    translate([0,-sheet_pos_z]) {
      ...
      translate([z_brace_sheet_pos_x,z_brace_sheet_pos_z,0]) {
        rotate([0,0,90]) {
          box_holes_for_side(z_brace_height,4);
        }
      }
    }
    ...
    brace_side_hole_x = z_brace_sheet_pos_x + z_motor_side * (sheet_thickness/2 + bc_shoulder_width + rounded_diam/2);
    other_side_hole_x = z_motor_side*(main_hole_width/2-rounded_diam/2);
    if (main_hole_height) {
      hull() {
        ...
        translate([0,-side_sheet_height/2+height_below_top_sheet/2]) {
          for(x=[brace_side_hole_x,other_side_hole_x]) {
            for(y=[front,rear]) {
              translate([x,(main_hole_height/2-rounded_diam)*y]) {
                accurate_circle(rounded_diam,resolution);
              }
            }
          }
...
module line_bearing_body(side=top,height=(line_bearing_mount_thickness+line_bearing_nut_diam/2+spacer)) {
  hull() {
    translate([0,side*(height-line_bearing_mount_thickness/2),0]) {
      line_bearing_body_base();
    }
    hole(bearing_body_diam,bearing_body_thickness,resolution);
  }
}

module line_bearing_holes(side=top,height=(line_bearing_mount_thickness+line_bearing_nut_diam/2+spacer)) {
  hole(line_bearing_diam+spacer*2,line_bearing_thickness+line_bearing_washer_thickness*2,resolution);
  hole(line_bearing_inner,bearing_body_thickness+0.5,8);

  translate([0,0,-bearing_body_thickness/2*side]) {
    rotate([0,0,90]) {
...
module front_xy_endcap() {
  ...
  bearing_y = front*(line_bearing_mount_thickness+line_bearing_nut_diam/2+spacer);
  ...
  module to_carriage_bearing_pos() {
    ...
    pos_zero() {
      translate([xy_line_x,bearing_y,to_front_line_z]) {
  ...
  module body() {
    hull() {
      // rod body
      translate([0,-line_bearing_mount_thickness/2,0]) {
        rotate([90,0,0]) {
          hole(y_rod_diam+wall_thickness*3,line_bearing_mount_thickness,resolution);
        }
      }
      translate([0,-line_bearing_mount_thickness/2,0]) {
        ...
        translate([endcap_side_screw_hole_pos_x,0,endcap_side_screw_hole_pos_z]) {
          rotate([90,0,0]) {
            hole(bc_screw_diam+wall_thickness*3,line_bearing_mount_thickness,resolution);
          }
        }
      ...
      to_carriage_bearing_pos() {
        translate([0,-bearing_y-line_bearing_mount_thickness/2,0]) {
          line_bearing_body_base();
        }
      }
      to_rear_bearing_pos() {
  ...
  module holes() {
    ...
    pos_zero() {
      translate([xy_line_x,0,to_front_line_z]) {
        rotate([90,0,0]) {
          hole(3,60,resolution);
          bevel_hole(3,.5,resolution);
        }
      }
      translate([return_line_x,0,return_line_z]) {
        rotate([90,0,0]) {
          hole(3,60,resolution);
          bevel_hole(3,.5,resolution);
        }
    ...
    // y rod
    rotate([90,0,0]) {
      % hole(y_rod_diam,line_bearing_mount_thickness*2+1,16);
    }
  }
...
module rear_xy_endcap(side) {
  ...
  high_bearing_y = line_bearing_mount_thickness + line_bearing_nut_diam/2 + spacer;
  mid_bearing_y  = high_bearing_y;
  ...
  low_high_dist_x          = xy_line_x+return_line_x;
  low_high_dist_z          = to_rear_line_z - return_line_z;
  high_to_low_line_angle_y = -atan2(low_high_dist_z,low_high_dist_x);
  ...
  module body() {
    hull() {
      ...
      high_bearing_pos() {
        translate([0,-high_bearing_y+line_bearing_mount_thickness/2,0]) {
          line_bearing_body_base();
        }
      }
      mid_bearing_pos() {
        translate([0,-mid_bearing_y+line_bearing_mount_thickness/2,0]) {
          line_bearing_body_base();
        }
      }
    }
  ...
  module holes() {
    ...
    // line holes
    pos_zero() {
      translate([xy_line_x,0,to_rear_line_z]) {
        rotate([90,0,0]) {
          hole(3,60,resolution);
          bevel_hole(3,.5,resolution);
        }
...
module z_brace_sheet() {
  belt_pos = front*(sheet_pos_y - sheet_thickness/2 - z_rod_pos_y);

  module body() {
    translate([0,-top_rear_brace_depth/2,0]) {
      box_side([z_brace_height,top_rear_brace_depth],[3,0,0,0]);
    }
  }

  module holes() {
    translate([z_brace_height/2-z_belt_anchor_height/2,0,0]) {
      for(side=[top,bottom]) {
        translate([z_belt_anchor_hole_spacing/2*side,belt_pos+tensioner_belt_dist_y,0]) {
          accurate_circle(nema17_screw_diam,resolution);
        }
      }
    }
...
module z_axis() {
  ...
  translate([0,z_rod_pos_y,build_base_z-z_printed_portion_height/2]) {
    for(side=[left,right]) {
      ...
      translate([z_bed_support_pos_x*side,-z_bearing_body_diam/2-z_build_platform_depth/2-0.1,0]) {
        rotate([0,0,-90]) {
          rotate([90,0,0]) {
            color("purple") linear_extrude(height=sheet_thickness,center=true) {
              z_support_arm();
            }
          }
        }
  ...
  translate([0,z_rod_pos_y-z_bearing_body_diam/2-z_bed_support_mount_depth-sheet_thickness/2,build_base_z-z_printed_portion_height/2]) {
    rotate([90,0,0]) {
      color("orange", sheet_opacity) linear_extrude(height=sheet_thickness,center=true) {
        z_main_plate();
      }
    }
