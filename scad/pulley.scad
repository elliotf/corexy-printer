...
module xy_motor_pulley_shaft_idler(idler_diam,groove_dist,num_turns,groove_diam) {
  difference() {
    xy_motor_pulley(idler_diam,groove_dist,num_turns,groove_diam);

    translate([0,0,-add_shaft_len/2]) {
      hole(idler_shaft_diam,100,16);
    }
  }
}

module xy_motor_pulley_bearing_idler(pulley_diam,groove_dist,num_turns,groove_diam) {
  difference() {
    union() {
...
translate([0,0,0]) {
  ...
  //translate([-31/2,-31/2,groove_spacing/2]) xy_motor_pulley_bearing_idler(10+4,groove_first_to_last_dist,turns,groove_diam);
  translate([31/2,-31/2,groove_spacing/2]) xy_motor_pulley_shaft_idler(idler_shaft_diam+6,groove_first_to_last_dist,turns,groove_diam);
}
